...
pdb_config_file=${PDB_CLUSTERWARE_PDB_CONFIG:-"$config_file"}
...
placement, also set RAC_SERVICE_PREFERRED to the comma-separated RAC instance
...
pdb_service_name() {
    ...
    local -a common_prefixes=(MAIN_PDB DEV_CLONE_1 DEV_CLONE_2 DEV_CLONE_CHILD QA_PDB)
    local -a lab_prefixes=(LAB03_TARGET_PDB LAB03_DIRECT_TARGET_PDB)
    local prefix

    for prefix in "${common_prefixes[@]}"; do
        configured_pdb=$(config_value "$prefix")
    ...
    for prefix in "${lab_prefixes[@]}"; do
        configured_pdb=$(pdb_config_value "$prefix")
        if [ "$pdb_name" = "$configured_pdb" ]; then
            configured_service="${configured_pdb}_SVC"
            printf '%s\n' "$configured_service"
            return 0
        fi
    done
...
if [ ! -f "$pdb_config_file" ]; then
    echo "ERROR: PDB configuration file was not found: ${pdb_config_file}" >&2
    exit 2
fi
