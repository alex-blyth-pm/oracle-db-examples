...
## Tested VM Cluster and CDB Memory Configuration

The labs were tested with 64 GB of Linux memory per database-server VM and a
kernel huge-page setting of 12,800 pages. The tested CDB memory settings were:
...
These values are a tested lab baseline, not sizing guidance for a production
workload. Size the VM cluster and CDB memory for the number of CDBs, PDBs, and
concurrent clones that your environment will run.

Check the configured values from `CDB$ROOT`:
...
Check Linux memory and huge pages on each database-server VM:
...
Lab 03 uses database links. Confirm both parameters are nonzero on every RAC
instance in both CDBs. `OPEN_LINKS=1` supports the lab, although `4` or higher
is recommended to retain capacity for other remote connections in the session:
...
Maintain regular RMAN backups and archived-log cleanup when retaining CDBs after
the lab. This preserves recovery-area capacity for normal work and remote
cloning.

### Lab Backup and FRA Cleanup Helper

For a simple disk-based backup of a lab CDB, use
[`tools/backup-cdb-and-clean-fra.sh`](../tools/backup-cdb-and-clean-fra.sh).
Run it as the Oracle software owner from a database server, with `ORACLE_SID`
set for the CDB. With an FRA configured and no RMAN `FORMAT` override, RMAN
writes the disk backup to the FRA. The script backs up the CDB, archived redo
logs, control file, and SPFILE, then deletes archived redo logs only after RMAN
confirms that they have been backed up once to disk. After cleanup, it lists the
RMAN backup summary and reports FRA capacity and file-type usage.

```bash
export ORACLE_SID=sourcecdb1
./tools/backup-cdb-and-clean-fra.sh
```

Run the helper separately for the source and target CDBs when retaining both
after Lab 03. In a RAC environment with archived logs stored locally on each
database server, configure RMAN channels that can access every archived-log
destination before using the helper. The script uses the current RMAN retention
and archived-log deletion policies; it does not change them. Schedule it using
your normal enterprise scheduler or `cron` only after validating backup and
recovery procedures for the environment.
