-- Create a transactionally consistent PDB snapshot from SALES_MAIN.
--
-- This snapshot is a demonstration of the CONSISTENT snapshot variation. The
-- later clone steps continue to use SALES_WEEKLY_SNAP.
...
PROMPT Creating consistent snapshot &&CONSISTENT_SNAPSHOT_NAME for &&MAIN_PDB
...
BEGIN
    ...
    IF l_main_pdbs = 0 THEN
        raise_application_error(
            -20001,
            'Cannot create snapshot &&CONSISTENT_SNAPSHOT_NAME because &&MAIN_PDB does not exist.'
        );
    END IF;
    ...
    AND    snapshot_name = UPPER('&&CONSISTENT_SNAPSHOT_NAME');

    IF l_snapshots > 0 THEN
        raise_application_error(
            -20002,
            'Cannot create snapshot &&CONSISTENT_SNAPSHOT_NAME because it already exists for &&MAIN_PDB.'
        );
    END IF;
END;
/
...
PROMPT   ALTER PLUGGABLE DATABASE SNAPSHOT &&CONSISTENT_SNAPSHOT_NAME CONSISTENT;
PROMPT   ALTER SESSION SET CONTAINER = &&ROOT_CONTAINER;

ALTER SESSION SET CONTAINER = &&MAIN_PDB;

ALTER PLUGGABLE DATABASE SNAPSHOT &&CONSISTENT_SNAPSHOT_NAME CONSISTENT;
...
PROMPT Consistent PDB snapshot created
