...
BEGIN
    ...
    IF l_container_name <> '&&ROOT_CONTAINER' THEN
        raise_application_error(-20000, 'Run this script from &&ROOT_CONTAINER. Current container is ' || l_container_name);
    END IF;

    IF UPPER(l_cdb_name) <> UPPER('&&LAB03_TARGET_CDB') THEN
        raise_application_error(-20032, 'Expected target CDB &&LAB03_TARGET_CDB. Connected to ' || l_cdb_name);
    END IF;

    SELECT COUNT(*) INTO l_pdb_count FROM dba_pdbs
...
PROMPT Creating direct thin clone &&LAB03_DIRECT_TARGET_PDB from &&MAIN_PDB@&&LAB03_SOURCE_DB_LINK
PROMPT SQL/DDL:
PROMPT   CREATE PLUGGABLE DATABASE &&LAB03_DIRECT_TARGET_PDB
PROMPT     FROM &&MAIN_PDB@&&LAB03_SOURCE_DB_LINK
PROMPT     SNAPSHOT COPY;

CREATE PLUGGABLE DATABASE &&LAB03_DIRECT_TARGET_PDB
    FROM &&MAIN_PDB@&&LAB03_SOURCE_DB_LINK
    SNAPSHOT COPY;

PROMPT PASS: Created &&LAB03_DIRECT_TARGET_PDB in MOUNTED mode.
